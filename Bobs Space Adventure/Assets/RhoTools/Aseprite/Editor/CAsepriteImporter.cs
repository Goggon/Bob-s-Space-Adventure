...
namespace RhoTools.Aseprite
{
    ...
    public class CAsepriteImporter
    {
        ...
        static bool _bindingSet = false;
        static EditorCurveBinding Binding
        {
            get
            {
                if (!_bindingSet)
        ...
        public static void ImportSheet(TextureImporter aTexture, CAsepriteWindow.Environment aEnv)
        {
            // Change metadata
            JSONNode tData = JSON.Parse(aEnv.targetAtlas.text);
            ...
            Vector2 tPivot = aEnv.pivot;
            int tAlignment = aEnv.alignment;
            for (int i = 0; i < tKeys.Count; i++)
            {
                ...
                if (tTag != "")
                {
                    ...
                    tThisData.alignment = tAlignment;
        ...
        public static AnimationClip[] ImportAnimation(TextureImporter aTexture, ref CAsepriteWindow.Environment aEnv)
        {
            JSONNode tData = JSON.Parse(aEnv.targetAtlas.text);
            JSONObject tFrames = tData["frames"].AsObject;
            aTexture.textureType = TextureImporterType.Sprite;
            ...
            EditorCurveBinding tBinding = GetBinding(aEnv.animType);
            AnimationClip[] tClips = new AnimationClip[tAnimations.Keys.Count];
            int j = 0;
            foreach (string tKey in tAnimations.Keys)
            {
                int tInd = ClipInList(aEnv.clips, tKey);
                AnimationClip tClip;
                if (tInd >= 0)
                    tClip = aEnv.clips[tInd];
                else
                ...
                if ((!aEnv.useConfig && aEnv.loopAnim)
                    || (aEnv.useConfig && !HasOption(tKey, NO_LOOP)))
                {
                ...
                if (aEnv.targetAnimator != null)
                {
                    ...
                    tAnimPath = Path.GetDirectoryName(tPath) + "/" + tKey + ".anim";
                }
                else
                {
        ...
        static EditorCurveBinding GetBinding(CAsepriteWindow.AnimationType aType)
        {
            EditorCurveBinding curveBinding = new EditorCurveBinding();

            // Set type
            if (aType == CAsepriteWindow.AnimationType.SpriteRenderer)
                curveBinding.type = typeof(SpriteRenderer);
            else if (aType == CAsepriteWindow.AnimationType.Image)
                curveBinding.type = typeof(UnityEngine.UI.Image);

            curveBinding.propertyName = "m_Sprite";
            if (CAsepriteWindow.UseChild)
        ...
        static EditorCurveBinding GetBinding()
        {
            return GetBinding(CAsepriteWindow.AnimType);
        }
